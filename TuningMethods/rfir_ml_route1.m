function [EstInfo] = rfir_ml_route1(data, n, kernel, T)
...
hpini = ini_rfir(y, Psi_p, kernel, n);
ff = @(x)nglglklhd(x, y, Psi_p, kernel, n);
options = optimoptions('fmincon', 'Display', 'off');
hp = fmincon(ff,hpini,[],[],[],[],lb,ub,[],options);
cost = nglglklhd(hp, y, Psi_p, kernel, n);
...
function [hpini] = ini_rfir(y, Psi_p, kernel, n)
        ...
        for nlam  = 1:Llam
            for nrho = 1:Lrho
                for nsigsqr = 1:Lsigsqr
                    ...
                    obj(nc,nlam,nrho,nsigsqr) = nglglklhd(hpstart, y, Psi_p, kernel, n);
                end
            end
        end
    end
    ...
    for nc = 1:Lc
        for nlam  = 1:Llam
                for nsigsqr = 1:Lsigsqr
                    ...
                    obj(nc,nlam,nsigsqr) = nglglklhd(hpstart, y, Psi_p, kernel, n);
                end
        end
    end
    [~, indx] = min(obj(:));
...
function [obj] = nglglklhd(hyper, y, Psi_p, kernel, n)
...
%Empirical Bayes
obj = method1_quadratic_form_inv(Psi_p, Nd, n, y, U', V', sigsqr) ...
    + method1_logdeterminant(Psi_p, Nd, n, U', V', sigsqr);
end
