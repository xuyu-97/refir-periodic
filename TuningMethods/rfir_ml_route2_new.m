function [EstInfo] = rfir_ml_route2_new(data, n, kernel, T)
...
[phity, R, le] = method2simu_fix(Psi_p, Nd, n, y);

tic
hpini = ini_rfir(phity, R, le, kernel, n, Nd);
time_ini = toc;

ff = @(x)nglglklhd(x, phity, R, le, kernel, n, Nd);
options = optimoptions('fmincon', 'Display', 'off');
hp = fmincon(ff,hpini,[],[],[],[],lb,ub,[],options);
cost = nglglklhd(hp, phity, R, le, kernel, n, Nd);
...
function [hpini] = ini_rfir(phity, R, le, kernel, n, Nd)
        ...
        for nlam  = 1:Llam
            for nrho = 1:Lrho
                for nsigsqr = 1:Lsigsqr
                    ...
                    obj(nc,nlam,nrho,nsigsqr) = nglglklhd(hpstart, phity, R, le, kernel, n, Nd);
                end
            end
        end
    end
    ...
    for nc = 1:Lc
        for nlam  = 1:Llam
                for nsigsqr = 1:Lsigsqr
                    ...
                    obj(nc,nlam,nsigsqr) = nglglklhd(hpstart, phity, R, le, kernel, n, Nd);
                end
        end
    end
    [~, indx] = min(obj(:));
...
function [obj] = nglglklhd(hyper, phity, R, le, kernel, n, Nd)
...
%Empirical Bayes
Ut = U'; Vt = V';
% obj = quadratic_form_inv(Psi_p, Nd, n, y, U', V', sigsqr) ...
%     + logdeterminant(Psi_p, Nd, n, U', V', sigsqr);
obj = method2simu_obj(Nd, R, phity, le, Ut, Vt, sigsqr);
